...
test('encodes and decodes chamber-108 RTP/Eframe datagrams', () => {
    ...
    assert.equal(frame[13], COMMAND_BUFFER_BYTES);
    assert.equal(frame.length, 14 + COMMAND_BUFFER_BYTES);
    assert.equal(frame.at(-1), 0);
    assert.equal(frame.subarray(14, 25).toString('ascii'), 'RP Track Up');
    assert.ok(frame.subarray(25).every((byte) => byte === 0));
...
test('zero-fills every non-character byte in the command payload', () => {
