...
- `/tuner` for tuning, stops, naming, and recorder controls. OSC-only organs use current-track/folder navigation and the controller naming keyboard; UDP-enabled organs also offer full-text renaming, name queries, direct folder selection, and pause. Playback, recording, protection, track stepping, and direct track playback use the available transport automatically. Faders appears only after named expression channels are received, and disconnected controls cannot queue commands for replay.
- `/console#probes` for locally broadcast environment, power, and wind-pressure readings
- `/console` uses one shared layout on phones, inlaid touchscreens, tablets, and desktops. Overview shows memory, current track, and transposer as full-width cards on phones, with the multi-column layout retained on tablets and desktops, alongside probes and expression; use the inline controls, or tap memory and expression readouts to open their detailed controls. Headers, navigation, and contextual action footers remain visible while the workspace scrolls. Tracks keeps record, stop, pause, play, and selected-track playback in its footer, with separate list, rename, and current-track panes. Memory and Expression have their own tabs for memory selection and expression sliders. Tabs size to their labels and wrap onto visible rows on narrow screens. Transposer controls are available on Overview. Crescendo has no separate remote API panel; explicitly configured site controls remain in custom views. Responsive rules adjust spacing and columns without replacing views. Touch controls are at least 48 CSS pixels high. The landing text is unchanged, with role choices stacked on narrow screens. Probes retains last readings with explicit stale/disconnected status and diagnostics. Settings includes Fugara pairing and Advanced controls.
