...
function parseProbeBroadcast(packet, remote = {}, receivedAt = new Date()) {
    ...
    const reportingFrequency = finiteNumber(
        payload.reportingFrequency ?? payload.updateFrequency,
        'reporting frequency',
        1,
        3600
    );
    if (!Number.isInteger(reportingFrequency)) {
        throw new Error('Invalid probe reporting frequency');
    }
