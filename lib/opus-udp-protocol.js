...
function encodeEframe(payload, sequence = 0, timestamp = Date.now()) {
    ...
    const frame = Buffer.alloc(14 + payloadBuffer.length);
    frame[0] = 0x80;
...
function fixedWidthRenameName(value) {
    ...
    // The controller's Set_Track_Name/Set_Folder_Name routines assign into a
    // 16-byte ARRAY OF CHAR. The UDP parser sizes its temporary array from the
    // quoted text, so short names must be space-padded to 15 visible bytes;
    // the parser supplies the sixteenth (0X terminator) byte.
    return name.padEnd(MAX_RENAME_CHARS, ' ');
}
...
function buildRemoteCommands(request, limits = {}) {
    ...
    case 'renameTrack': {
        ...
        commands = [
            `CA Rename Track ${number} "${fixedWidthRenameName(request.name)}"`
        ];
        break;
    }
    case 'renameFolder': {
        ...
        commands = [
            `CA Rename Folder ${number} "${fixedWidthRenameName(request.name)}"`
        ];
        break;
    }
