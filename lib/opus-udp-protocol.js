...
function encodeEframe(payload, sequence = 0, timestamp = Date.now()) {
    ...
    // Include the Oberon end-of-string marker in the Eframe payload. Although
    // the controller's UDP receiver is intended to append this byte, some
    // firmware builds do not, leaving string operations (notably rename)
    // reading beyond the received command.
    const terminatedPayloadLength = payloadBuffer.length + 1;
    const frame = Buffer.alloc(14 + terminatedPayloadLength);
    frame[0] = 0x80;
    ...
    frame[13] = terminatedPayloadLength;
    payloadBuffer.copy(frame, 14);
    // Buffer.alloc() zero-fills the final byte.
    return frame;
}
