...
class OpusUDPTransport {
    constructor(options = {}) {
        ...
        this.directAdvertisementHost = null;
        this.lastDirectAdvertisementAt = 0;
        this.replyHost = null;
        this.lastReplyAt = 0;
        ...
        this.commandSocket.on('message', (message, rinfo) => {
            ...
            if (
                ...
                this.replyHost = rinfo?.address ?? null;
    ...
    get hasReplied() {
        return this.targetHost !== null && this.replyHost === this.targetHost
            && Date.now() - this.lastReplyAt <= this.controllerTimeoutMs;
    }
    ...
    _maintainConnection(now = Date.now()) {
        ...
        this.advertise();
        // A read-only heartbeat establishes API readiness independently of
        // OSC/SSDP discovery, and lets readiness expire if replies stop.
        if (this.targetHost !== null && !this.closing) this.send('Query OLED');
    }
    ...
    _sendDatagram(datagram) {
        ...
        this._advertiseToController(datagram.host);
    ...
    _advertiseToController(host = this.targetHost) {
        if (!host || !this.commandReady || this.closing) return;
        const now = Date.now();
        if (host === this.directAdvertisementHost
            && now - this.lastDirectAdvertisementAt < this.beaconIntervalMs) return;

        // Use the command socket so registration and commands take the same
        // route/source IP, including when OSC found a controller but multicast
        // discovery cannot reach it. Keep multicast announcements as well.
        this.directAdvertisementHost = host;
        this.lastDirectAdvertisementAt = now;
        this.commandSocket.send(buildSSDPBeacon(this.uuid), SSDP_PORT, host, error => {
            if (error) {
                this.directAdvertisementHost = null;
                this.onError(error);
            }
